...
@Component({
  selector: "app-aws-transcription",
  templateUrl: "./aws-transcription.component.html",
  styleUrls: ["./aws-transcription.component.scss"]
})
export class AwsTranscriptionComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private processCaptionLines(lines: string[], start: number | undefined, end: number) {
    if (start) {
      ...
      const timestamp = Number.parseInt(parts[0]) * 3600 + Number.parseInt(parts[1]) * 60 + Number.parseFloat(parts[2]);
      this.captions.push({ timestamp: timestamp, lines: textLines });
    }
  }
