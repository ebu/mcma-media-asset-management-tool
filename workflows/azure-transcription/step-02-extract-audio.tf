...
resource "aws_iam_role" "step_02_extract_audio" {
  name               = format("%.64s", "${var.prefix}-${var.aws_region}-02-extract-audio")
...
resource "aws_iam_role_policy" "step_02_extract_audio" {
  name   = aws_iam_role.step_02_extract_audio.name
  role   = aws_iam_role.step_02_extract_audio.id
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${aws_lambda_function.step_02_extract_audio.function_name}:*",
...
resource "aws_lambda_function" "step_02_extract_audio" {
  ...
  environment {
    variables = {
      ...
      ACTIVITY_ARN                    = aws_sfn_activity.step_02_extract_audio.id
    }
  }
...
resource "aws_sfn_activity" "step_02_extract_audio" {
  name = "${var.prefix}-02-extract-audio"
