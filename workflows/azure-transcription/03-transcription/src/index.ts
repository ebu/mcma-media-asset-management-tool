...
const loggerProvider = new AwsCloudWatchLoggerProvider("azure-transcription-03-transcription", process.env.LogGroupName);
...
export async function handler(event: InputEvent, context: Context) {
    ...
    try {
        ...
        const [jobProfile] = await resourceManager.query(JobProfile, { name: "AzureTranscription" });
        if (!jobProfile) {
            throw new McmaException("JobProfile 'AzureTranscription' not found.");
        }
