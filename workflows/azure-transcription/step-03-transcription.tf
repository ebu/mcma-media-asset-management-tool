    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:logs:${var.aws_region}:${var.aws_account_id}:log-group:/aws/lambda/${aws_lambda_function.step_03_transcription.function_name}:*",
...
resource "aws_lambda_function" "step_03_transcription" {
  ...
  layers = var.enhanced_monitoring_enabled ? ["arn:aws:lambda:${var.aws_region}:580347275435:layer:LambdaInsightsExtension:14"] : []
  ...
  environment {
    variables = {
      ...
      ActivityArn      = aws_sfn_activity.step_03_transcription.id
    }
  }
