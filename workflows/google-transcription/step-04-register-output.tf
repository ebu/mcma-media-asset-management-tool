...
resource "aws_iam_role" "step_04_register_output" {
  name               = format("%.64s", "${var.prefix}-${var.aws_region}-04-register-output")
...
resource "aws_iam_role_policy" "step_04_register_output" {
  name   = aws_iam_role.step_04_register_output.name
  role   = aws_iam_role.step_04_register_output.id
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
