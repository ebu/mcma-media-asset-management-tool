...
resource "aws_iam_role" "step_02_transcription" {
  name               = format("%.64s", "${var.prefix}-${var.aws_region}-02-transcription")
...
resource "aws_iam_role_policy" "step_02_transcription" {
  name   = aws_iam_role.step_02_transcription.name
  role   = aws_iam_role.step_02_transcription.id
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${aws_lambda_function.step_02_transcription.function_name}:*",
...
resource "aws_lambda_function" "step_02_transcription" {
  function_name    = format("%.64s", "${var.prefix}-02-transcription")
  role             = aws_iam_role.step_02_transcription.arn
  handler          = "index.handler"
  filename         = "${path.module}/02-transcription/build/dist/lambda.zip"
  source_code_hash = filebase64sha256("${path.module}/02-transcription/build/dist/lambda.zip")
  runtime          = "nodejs18.x"
  timeout          = "900"
  memory_size      = "2048"
  ...
  environment {
    variables = {
      ...
      ACTIVITY_ARN                    = aws_sfn_activity.step_02_transcription.id
    }
  }
...
resource "aws_sfn_activity" "step_02_transcription" {
  name = "${var.prefix}-02-transcription"
