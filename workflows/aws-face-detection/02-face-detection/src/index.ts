...
const loggerProvider = new AwsCloudWatchLoggerProvider("aws-face-detection-02-face-detection", process.env.LogGroupName);
...
export async function handler(event: InputEvent, context: Context) {
    ...
    try {
        ...
        const [jobProfile] = await resourceManager.query(JobProfile, { name: "AwsFaceDetection" });
        if (!jobProfile) {
            throw new McmaException("JobProfile 'AwsFaceDetection' not found.");
        }
