...
const loggerProvider = new AwsCloudWatchLoggerProvider("aws-label-detection-02-label-detection", process.env.LogGroupName);
...
export async function handler(event: InputEvent, context: Context) {
    ...
    try {
        ...
        const [jobProfile] = await resourceManager.query(JobProfile, { name: "AwsLabelDetection" });
        if (!jobProfile) {
            throw new McmaException("JobProfile 'AwsLabelDetection' not found.");
        }
