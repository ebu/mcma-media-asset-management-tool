...
resource "aws_iam_role" "step_03_register_output" {
  name               = format("%.64s", "${var.prefix}-${var.aws_region}-03-register-output")
...
resource "aws_iam_role_policy" "step_03_register_output" {
  name   = aws_iam_role.step_03_register_output.name
  role   = aws_iam_role.step_03_register_output.id
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${aws_lambda_function.step_03_register_output.function_name}:*",
...
resource "aws_lambda_function" "step_03_register_output" {
  filename         = "${path.module}/03-register-output/build/dist/lambda.zip"
  function_name    = format("%.64s", "${var.prefix}-03-register-output")
  role             = aws_iam_role.step_03_register_output.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("${path.module}/03-register-output/build/dist/lambda.zip")
  runtime          = "nodejs16.x"
  timeout          = "900"
  memory_size      = "2048"
