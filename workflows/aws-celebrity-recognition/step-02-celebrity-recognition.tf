#################################
#  Step 2 : Celebrity Recognition
#################################

resource "aws_iam_role" "step_02_celebrity_recognition" {
  name               = format("%.64s", "${var.prefix}-${var.aws_region}-02-celebrity-recognition")
...
resource "aws_iam_role_policy" "step_02_celebrity_recognition" {
  name   = aws_iam_role.step_02_celebrity_recognition.name
  role   = aws_iam_role.step_02_celebrity_recognition.id
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [
      {
...
resource "aws_lambda_function" "step_02_celebrity_recognition" {
  filename         = "${path.module}/02-celebrity-recognition/build/dist/lambda.zip"
  function_name    = format("%.64s", "${var.prefix}-02-celebrity-recognition")
...
resource "aws_sfn_activity" "step_02_celebrity_recognition" {
  name = "${var.prefix}-02-celebrity-recognition"
